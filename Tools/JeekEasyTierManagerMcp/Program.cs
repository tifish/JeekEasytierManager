...
using JeekEasyTierManager;

// JeekEasyTierManager MCP stdio adapter.
//
...
//
//   { "command": "C:\\...\\bin\\JeekEasyTierManagerMcp.exe", "args": ["--surface", "debug"] }
//
...
while (await stdin.ReadLineAsync().ConfigureAwait(false) is { } line)
{
    ...
    {
        await stdout
            .WriteLineAsync(AdapterText.RpcError(null, -32700, $"Parse error: {ex.Message}").ToJsonString())
...
JsonNode OfflineResponse(string? method, JsonNode? id, string reason) =>
    method switch
    {
        ...
        "tools/call" => AdapterText.RpcResult(
            ...
            new JsonObject
            {
                ["content"] = new JsonArray(
                    ...
                    {
                        ["type"] = "text",
                        ["text"] = $"JeekEasyTierManager is not reachable on {options.DescribePipe()}. "
                            + $"Start the app (a Debug build for the debug surface) and retry. Details: {reason}",
                    }),
                ["isError"] = true,
            }),
        _ => AdapterText.RpcError(
            id,
            -32601,
            $"Method not available while JeekEasyTierManager is closed: {method}"),
    };
...
internal sealed record AdapterOptions(string PipeName, string Surface)
{
    public string ServerName => "jeek-easytier-manager-debug";
    ...
    public static AdapterOptions Parse(string[] args)
    {
        ...
        var pipeName =
            pipe
            ?? McpPipeNames.Resolve(
                surface,
                instance ?? McpPipeNames.InstanceId(AppContext.BaseDirectory));
        return new AdapterOptions(pipeName, surface);
    }
}
