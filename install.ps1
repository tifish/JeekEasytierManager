# JeekEasyTierManager one-click installer.
# Usage:
...
#
# No registry writes. To uninstall: quit the app, uninstall the services from the app first,
# then delete %LOCALAPPDATA%\Programs\JeekEasyTierManager and the Start Menu shortcut.
...
#    version no longer ships while preserving user data and files the running
#    EasyTier services keep locked. The exclusion list matches the auto-update
#    script (bin/AutoUpdate.ps1).
Write-Host "Installing to $InstallDir"
robocopy $stageDir $InstallDir /MIR /XD Config Settings Logs EasyTier Nssm /NFL /NDL /NJH /NJS /NP | Out-Null
...
# 6. Start the app; if the required .NET runtime is missing, Setup.cmd installs
...
if ($hasRuntime) {
    ...
    Write-Host ".NET 10 ASP.NET Core runtime not found; running Setup.cmd to install it..."
